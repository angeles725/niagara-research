...
| L-8 | **36-hour `generated` grace (anti-clock-rollback tolerance)** — `LicenseFile.java:110-113` allows `now < generated − 36 h` (129600000 ms) | probes with `generated=2026-08-02` (~28 h ahead) vs `generated=2026-08-04` (~52 h ahead), vendor=`Tridium`, attacker-signed | `+28 h` → **passed** the `generated` check (failed later at signature: `Invalid signature`); `+52 h` → rejected with `"Current date is earlier than license generated date"` | **CONFIRMED** — the anti-clock-rollback check has a deliberate 36-hour skew tolerance, verified live on both sides of the boundary |
| L-9 | `station.exe` gate without license | `station.exe` (no station configured) | Same F1: `FeatureNotLicensedException: tridium:nre` at `NLicenseManager.checkFeature(NLicenseManager.java:89)` | **CONFIRMED** — the station launcher is gated the same way as `plat.exe` |
| L-10 | **Workbench GUI boots without a license** | `wb.exe` GUI launch | Process stays **RUNNING** (pid observed, no exit within 8 s) — the Workbench launcher itself is NOT license-gated (only the tool/station classes are, per L-7/L-9) | **CONFIRMED** — Workbench starts; a real station session still requires the license (positive control pending) |
