...
addendum: 2026-08-05 (B366 build-vs-buy — does NOT reopen; focus stays 9/9. Niagara Analytics does not ship the banded PSI-vs-time chart out of the box: 7 analytical chart types, none a limit-band trend; 0 alarm/limit-band code in authored frontend; Analytics charts ON webChart so inherits its ceiling; official guide frames alarm vs chart as parallel outputs. Verdict: buying Analytics trims detection+aggregation legs but NOT the dominant banded-chart cost → confirms+strengthens B362. Raises the "webChart native bands?" thread → next block under B199 line, not a reports gap)
addendum: 2026-08-05 (B367 webChart-bands — closes the thread B366 opened; under B199 charting line, not a reports gap. webChart does NOT render alarm-limit bands natively either: only per-sample status coloring (isAlarm→color, no band); render engine is a HARDCODED 11-layer array in line/Line.js:25 with NO LimitLayer and NO addLayer/registry → band = fork the composition; whole-module sweep 0 limitLine/threshold/band; the {t,v,r,s} feed carries NO limit values (structural clincher); docDeveloper+niagara-help silent. VERDICT: the <12/>28 band is intrinsically CUSTOM on all 4 charting paths (report/classic BChart/Analytics/webChart) — confirms B362 a 5th way. webChart still cheapest route (inherit engine, fork a layer) but not because bands are native. Cost ordering: webChart-fork < BChart-subclass < headless-from-scratch. Charting-path question now FULLY closed)
...
| B366 | — (addendum) | no · inline driver reads (bounded build-vs-buy question) | ADDENDUM (not a gap). Would licensing Niagara Analytics deliver the client's banded PSI-vs-time chart, avoiding the custom renderer B362 priced as dominant? NO. BAnalyticsChartFactory implements webChart BIChartFactory over box:BoxTable, delegates to AnalyticsChartFactory.js (extends webChart BaseChartFactory) → Analytics is a webChart CONSUMER, inherits its ceiling. 7 chart types (remittance B66 §66.3.3), none a limit-band trend (Spectrum=histogram). Clean measured sweep of authored rc/**: 0 alarm / 0 setpoint / 0 alarm-limit / 0 limit-line / 0 region (threshold/limit hits = d3 scale + textLength; baseline=comparison concept). docAnalyticsGuide frames alert/alarm vs chart as parallel outputs, never band-overlay. Verdict: buy trims detection (algorithm→alarm record) + aggregation (trend wrappers) legs but NOT the banded chart → custom in both build+buy paths; confirms+strengthens B362. Opens the "does webChart itself render alarm-bound regions/y-grid-lines?" thread (next block, under B199 not reports). [CERT]×4 + remittance×2 + [CERT-doc]×1 + [INFER]×1, ratio 0.14 (EVIDENCE/DECISION). |
