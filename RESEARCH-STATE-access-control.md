...
status: stopped (8/8, investigable=0; AC1→B558 … AC8→B566). Runbooks (document-mode, NOT gaps): B560 = cloudflared remote access; B726 = SSH -L jump-host variant (2026-09-02, [CERT-live] Pancaddia León — Fox no-IP-redirect proof + Platform-only backup/commission path); B727 = expose station AS oBIX server to an external client (2026-09-03, ObixNetwork + Server slot + per-user HTTPBasicScheme + license `export`; [CERT-doc]+[CERT], cites B499/B509/B600); B728 = login-less landing on a web dashboard behind a public reverse proxy (2026-09-03, NO native anonymous/auto-logon in N4 → proxy injects `Authorization: Basic` for a dedicated read-only HTTPBasicScheme user + Home Page = dashboard; security envelope = Cloudflare Access + categories + firewall; field pattern Mercato/Hilton; [CERT]+[CERT-doc], cites B727/B510/B509/B561). §728.5 = relayed [CERT-live] end-to-end confirmation (Pancaddia León, JACE-9000 N4 4.15.3.28: GET /dashboardpan/ + injected Basic → 200, no auth → 302, Transform Rule late_transform; deployed OPEN/no-Access by user decision — recorded deviation). §18 retro DONE (retros/2026-09-03-obix-and-loginless-dashboard-runbooks-retro.md, review-status pending).
