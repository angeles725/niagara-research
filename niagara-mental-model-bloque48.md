...
**TODO 48-2**: Auditar `nmodsreflow-rt` exhaustivamente — listar handlers que reciben writes via `BIServerSideCallHandler` y verificar que cada uno hace `cx.getUser().getPermissionsFor(...).hasOperatorWrite()` antes de aplicar el cambio. Los `requiredPermissions="r"` en `@AgentOn` cubren solo la invocación del agent, no el efecto del handler.
