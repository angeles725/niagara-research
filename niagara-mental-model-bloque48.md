...
**TODO 48-5** ✅ RESOLVED 2026-05-07 (engram topic_key `niagara-mental-model/bloque48-todo5-resolved` + commit reflow-clean-177 `701ccfc`): Audit empírico confirmó la vulnerabilidad pero con **severidad MEDIUM con caveat empírico**. **Confirmado**: `BReflowUserCommands.getAllRoles()` (`BReflowUserCommands.java:55-58`) retornaba sin gating en body la lista completa de role IDs del station (`BRoleService.getEnabledRoleIds()` → CSV) a cualquier user con `r` en `ReflowService`. El uso legítimo es `UserProfileRoles.vue:65` (componente Reflow profiles config mode) que requiere checkboxes para asignar roles a un profile — mounteado solo en config UI que requiere `rw`, NO choca con el fix. **Caveat severidad**: el atacante con `r` ya puede potencialmente leer `BRoleService` via WB Property Sheet o baja directo si las permisos del RoleService lo permiten — sin runtime no se puede desambiguar si el BOX endpoint era bypass real o solo conveniencia (mismo data ya accesible nativo). **Fix aplicado**: body check `if (!cx.getUser().getPermissionsFor(comp).hasOperatorWrite()) throw new SecurityException(...)` — 3 líneas, mínimo invasivo, NO toca `getRoles` (la que retorna roles del user actual sigue siendo `r` para preservar el gating UI pattern del 48.10.1). **Patrón**: análogo al `canAcknowledgeAlarms:111` de Bloque 48 sección 9 (`hasOperatorWrite()` check), pero acá throw en vez de return boolean — porque acá denegamos invocación, no informamos al cliente para gating UI.
