...
| `campaign10-silent-protection-pattern-b` | S23 | the silent-protection surface follow must recognise the Pattern B adapter (`implements BIAlarmSource` + `newOffnormalAlarm`/`new AlarmSupport(`), not only Pattern A `BAlarmSourceExt` | types/logic.md:102 (the lint mention under §Protection anatomy :98) — add "the lint recognises BOTH patterns' surfaces (A: BAlarmSourceExt on a point; B: the B-adapter implementing BIAlarmSource)" |
...
6. **A heuristic pairing a flag with a call needs the shared method-boundary parser** — S21: a forward brace-scan false-fired on a method-local; the section-D parser + FIELD=depth-1 is the reusable shape (S22/S23 reuse it).
