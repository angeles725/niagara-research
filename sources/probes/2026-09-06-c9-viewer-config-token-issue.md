...
1. Before the first write in a session, `POST <write-server>/config/login` with `{ "password": "<the shared config password>" }`
   and the usual `Authorization: Bearer <supabase-jwt>`. → **200** `{ "token": "<opaque>" }` on the right password; **401**
   (no `token`) on a wrong one — show the modal, let the operator retry.
...
3. TTL is **10 min sliding** (server `CONFIG_TTL_MS=600000`, refreshed on each accepted write). `POST /config/logout`
   (with the token) ends the session immediately; the next write with that token is 403.
...
|---|---|---|---|
| `POST /config/login` | `{password}` + Bearer | 200 `{token}` | 401 (wrong password, no token) |
...
- The chip/countdown already in the `--config-login` mock reflects the real `/config/session` remaining TTL.
...
| 3 | TTL 10 min sliding, header not cookie | [CERT] | R5 package (CONFIG_TTL_MS 600000); dashboard-preview --config-login |
