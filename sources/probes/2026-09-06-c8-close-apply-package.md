...
### Step B — the recommended folded-as-code lines (already token-credited; convention completeness, non-blocking)

Paste each from the close-fold drafts verbatim (target `METHODOLOGY.md` §Conformance): §9 station-snapshot, §10
bog-audit, §11 wb-audit, §12 lint-servlet, §16 retro-loop, §17 orchestration (+ its L7/L8 numbering doctrine line),
§18 structure, §19 write-path, §20 station-logic. Full text: `2026-09-05-c8-close-fold-drafts.md` §9-§12, §16-§20. These
do not block the strict audit; skip only if the close PR is kept minimal, but preferred for the convention.
...
| 2026-09-06-campaign8-structure.md             | kit | … | folded | 3  |   ← RE-CHECK
| 2026-09-06-campaign8-write-path.md            | kit | … | folded | 5  |   ← RE-CHECK
| 2026-09-06-campaign8-station-logic.md         | kit | … | folded | 4  |   ← RE-CHECK
```
...
```
Commit message (conventional + the `Retro:` trailer grammar seen on niagara-tools, e.g. `bab15d2`):
```
docs(build-n4-module): campaign 8 close — fold 20 retros, extend CHANGELOG v0.19.0, retro_pending=0

<one-line summary of the fold set>

Retro: promotion (folds campaign8-{lint-delays,…,station-logic} + close-process-meta-lessons; 20 INDEX rows pending→folded; 5 mandatory lines A.1-A.5; BUILD-STATE kit retro_pending=false)
```
...
| 2 | The other 15 are token-credited (≥1 in-kit) | [CERT] | git grep per slug (§9-§12,§16-§20 counts) |
