...
| T2 | **client-read-root hygiene** — **10** bats hardcode an absolute `Leon-Guanjuato` client path (lead widened the RED to ALL such literals): 7 blessed-worktree defaults (5 `C9_CLIENT_ROOT` + 2 `C9_CLIENT_REPO` → `main-ff1b659`) + 3 LIVE-checkout (`C8_CLIENT_REPO` + 2 override-less `$HOME/…/Leon-Guanjuato`). Unify into ONE `tests/lib/client-root.bash` exporting all three vars from one default | KIT/test | Med-High (a moved tree needs 10 edits; 2 override-less can't be redirected today; the live-checkout reads violate the client-reads rule) | High (one helper + edit 10) | WSL | RED qa/c11-client-root (widened): 10 offenders → 0 |
...
| 2 | 10 absolute Leon-Guanjuato literals in tests/*.bats (7 blessed + 3 live-checkout), one per file | [CERT] | grep Leon-Guanjuato tests/*.bats (excl. lib) @ dab0807 = 10 |
