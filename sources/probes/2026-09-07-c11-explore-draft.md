...
| T2 | **client-read-root hygiene** — **7** bats hardcode the blessed-worktree default `main-ff1b659` under TWO var names (5 `C9_CLIENT_ROOT` + 2 `C9_CLIENT_REPO`), **+ a 3-bats tail on the LIVE main checkout** `…/Leon-Guanjuato` (2 with NO env override — the exact thing the client-reads rule warns against). Unify into ONE `tests/lib/client-root.bash`; PR7 `b6b65a2` hand-retargeted 3 at the C10 close — the churn T2 deletes | KIT/test | Med-High (a moved worktree needs 7 edits; 2 override-less bats can't be redirected at all) | High (one helper + edit 7, convert 2-3) | WSL | tests/client-root.bats — default resolves from one source; override wins |
...
| 2 | 7 blessed-worktree defaults (5 C9_CLIENT_ROOT + 2 C9_CLIENT_REPO, all main-ff1b659) + 3 live-checkout hardcodes (2 override-less) | [CERT] | grep C9_CLIENT_(ROOT\|REPO)=7; c8-close/lint-delays/rc-scan @ dab0807 |
