...
retros / wave3 spec cited inline. **UPDATE 2026-09-06:** wave 3 (PR16–PR20) is now ALL MERGED on main `17af2ed` — the
former placeholders are promoted to real entries below; re-confirm the exact bats/smoke counts against the shipped
scripts at apply (real counts over design estimates).
...
## Wave 3 (PR16–PR20) — ALL MERGED on main `17af2ed` (verified 2026-09-06; re-check exact bats/smoke counts at apply)

> Token counts on `origin/main`: retro-loop 8, orchestration 1, structure 4, write-path 4, station-logic 2 (all
> credited). All 20 INDEX rows still `pending` → the close PR flips them.

### Added — Campaign 8 close Wave 3: retro loop, structure/write-path/station-logic lints (PR16, PR18–PR20)

- **`new-retro.sh` + `kit-ticket.sh` (PR16, retro-loop):** `toolbelt/new-retro.sh <module-or-kit> [--ticket]` creates `retros/<date>-<slug>.md` from a template (line 1 `<!-- review-status: pending -->`, 4 sections), appends one `retros/INDEX.md` row, and sets `retro_pending: true` in `BUILD-STATE.md` — all three atomically (the flip is awk section-scoped so it never marks sibling module sections, RL7), idempotent on re-run (RL9 INDEX-guard), a 6-char slug floor so `sweep-fold-audit` can credit the token (RL8); `toolbelt/kit-ticket.sh "<title>" --from <retro-file>` opens a kit-repo GitHub issue (`gh issue create`, labels `kit,from-run`), SKIP+exit-0 when `gh` is absent/unauth (never fails a run); `skill/SKILL.md` close-of-run step + `BUILD-LOOP.md §7` retro gate; RL1–RL9 bats [ev: retro campaign8-retro-loop].
- **`lint-structure.sh` + `types/structure.md` (PR18, structure):** `toolbelt/lint-structure.sh <module-root>` implements the L1–L11 structure lint (L1 package naming, L2 one `@NiagaraType`/file, L3 pure-model package no-baja, L4 lexicon non-empty, L5 palette non-empty for rt, L6 `module-include.xml` consistent, L7 3-part dep floors, L9 no empty skeleton, L10 no absolute host paths in tracked `gradle.properties`, L11 mixed pure+Baja srcTest declares both `:test-wb` and junit; L8 signed-jar stays in `verify-module.sh`); profile discovery is by direct-child `*-(rt|ux|wb|se|doc)`/build-script — NOT by `module-include.xml`, so an emptiest artifact is still visited — and L10 walks parents to the `.git` sentinel; row `FAIL|WARN  lint-structure  <path>  L<n>: <reason>`; exits 0/1/3; `scaffold-module.sh` output passes L1–L11; `types/structure.md` B817 layout doctrine; LS4–LS11 + LS9-real + LS10-real bats [ev: retro campaign8-structure].
- **`lint-write-path.sh` (PR19, write-path):** `toolbelt/lint-write-path.sh <module-root> [--bog <config.bog>]` — every `Flags.OPERATOR` `@NiagaraProperty` MUST have a row in `docs/write-path-matrix.md` (slot · writer · timing · an existing `src/test` test name); walk-up matrix resolution + `--matrix` override; a MISSING matrix is an ERROR (exit 3), never a silent pass; `--bog` adds bog-derived slots; also adds `types/logic.md` §Write-path & overlap + `types/logic-authoring.md` §Write-path test matrix; WP1–WP8 bats [ev: retro campaign8-write-path] [ev: corpus B816].
- **`bog-audit.sh` CHECK13–CHECK19 (PR20, station-logic):** extends `toolbelt/bog-audit.sh` with the station-wiring checks — CHECK13 relay-double-source (FAIL: two sources into one BooleanWritable relay slot), CHECK14 own-output-unlinked (WARN: a type-inferred `*Out` output with no outgoing relay link; catches TRANSIENT outputs too, D5), CHECK15 sensor-crossed-by-name (WARN: a `C{n}` slot sourced from the wrong `E{m}` unit), CHECK16 hasDefrost↔DefrostController sibling (FAIL, BOTH directions), CHECK17 roomN-index-mismatch (FAIL: `ColdRoom_1` links addressing `evap3*`), CHECK18 evaporator-unit tile-number consistency (FAIL: HOA tile ≠ freeze tile), CHECK19 panel-writes-state-into-control (WARN); SL13–SL19 bats. NOTE: the QA smoke pins were tightened to count+subject+absence after the presence-only pins passed on the real bog (close-retro lesson 11(a)) [ev: retro campaign8-station-logic].

### Changed — Campaign 8 close Wave 3: orchestration doctrine (PR17)

- **Orchestration doctrine (PR17, doc-only):** `build-n4-module-kit/ORCHESTRATION.md` created (8 sections: roles, model table, delegation triggers, escalation gate, artifact store, evidence discipline, retro/ticket loop, recovery); `skill/SKILL.md` gains steps 1b (Explore shard, audit-first >3 files, sonnet), 1c (opus design shard for schema/new-slot/facade), 5b (peer QA RED-by-branch-name session before every code PR); `tests/kit-links.bats L8` (NOT L7 — L7 already existed from PR13 §6.a) verifies every ORCHESTRATION-named script exists at `toolbelt/<script>.sh` [ev: retro campaign8-orchestration].
