...
public class BRemoveListElementAction extends BAction {
   public static final Property parameterTypeSpec = newProperty(5, BTypeSpec.NULL, null);
   public static final Type TYPE = Sys.loadType(BRemoveListElementAction.class);
   ...
   public BValue invoke(BComponent target, BValue arg) {
      if (!(target instanceof BBacnetListOf)) {
         throw new IllegalArgumentException("RemoveListElement cannot be invoked on " + target.getType());
      } else {
         BBacnetListOf list = (BBacnetListOf)target;
         if (!arg.getType().is(list.getListType())) {
            throw new IllegalArgumentException("Invalid type " + arg.getType() + " for " + list);
         } else {
            list.removeListElement(arg, null);
