...
public class BBacnetEnumTrendLogExt extends BBacnetEnumIntervalTrendLogExt {
   ...
   public static final Type TYPE = Sys.loadType(BBacnetEnumTrendLogExt.class);
   ...
   public void changed(Property p, Context cx) {
      ...
      if (p.equals(interval)) {
         BStatusValue value = null;

         try {
            if (this.getRecord() != null) {
               value = this.getParentPoint().getOutStatusValue();
            ...
            log.info("Exception when point changed for the " + value);
         }
      }
   }
   ...
   protected boolean isChange(BStatusValue oldValue, BStatusValue newValue) {
      ...
      if (!oldValue.getStatus().equals(newValue.getStatus())) {
         ...
         BDynamicEnum newEnum = ((BStatusEnum)newValue).getValue();
         BDynamicEnum oldEnum = ((BStatusEnum)oldValue).getValue();
         return newEnum != oldEnum;
      }
   }
