...
public class BBacnetEnumCovTrendLogExt extends BEnumCovHistoryExt implements BIBacnetTrendLogExt {
   ...
   public void atSteadyState() {
      if (this.isRunning()) {
         try {
            ...
            log.log(Level.SEVERE, "Error storing log status enabled event", (Throwable)var13);
         } finally {
            this.trigger = false;
         }
      }
         ...
         if (p.equals(enabled)) {
            if (!this.isParentLegal(this.getParent().asComponent())) {
               ...
               try {
                  ...
                  log.log(Level.SEVERE, "Error storing event", (Throwable)var11);
               } finally {
                  this.trigger = false;
               }
            }
   ...
   public void clockChanged(BRelTime shift) throws Exception {
      if (this.getEnabled()) {
         try {
            ...
            log.log(Level.SEVERE, "Error storing event", (Throwable)var14);
         } finally {
            this.trigger = false;
         }
      }
   ...
   public void doStopLogging() {
      try {
         ...
         log.log(Level.SEVERE, "Error storing event", (Throwable)var9);
      } finally {
         this.trigger = false;
         this.setEnabled(false);
      }
