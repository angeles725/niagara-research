...
public class BOfflineConverterConfig extends BConverterConfig {
   ...
   public static final Type TYPE = Sys.loadType(BOfflineConverterConfig.class);
   ...
   public BOfflineConverterConfig(BBinding[] bindings, BObject to) {
      ...
      for (int i = 0; i < bindings.length; i++) {
         ...
         TypeInfo[] adapters = Sys.getRegistry().getAdapters(null, this.toType);
   ...
   public void doBindingComboChanged() {
      ...
      if (a != -1) {
         this.loadBinding(this.bindings[a]);
      }
   }
   ...
   public void doConverterComboChanged() {
      ...
      if (a != -1) {
         ...
         if (b != -1) {
            this.loadFe(this.conv[b]);
            this.relayout();
         }
      }
   }

   private void loadBinding(BBinding binding) {
      TypeInfo[] adapters = Sys.getRegistry().getAdapters(null, this.toType);
      ...
      for (int j = 0; j < adapters.length; j++) {
         ...
         this.conv[j].init(null, this.to);
         this.convCombo.getList().addItem(this.name(this.conv[j]));
      }

      this.convCombo.getList().setSelectedIndex(0);
      this.loadFe(this.conv[0]);
   }

   private void loadFe(BConverter c) {
      this.fe = this.makeFe(c);
   ...
   private BWbFieldEditor makeFe(BConverter c) {
      ...
      if (fe instanceof BIEnumToSimpleFE) {
         facets = BFacets.makeEnum(BEnumRange.NULL);
      }
