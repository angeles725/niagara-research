...
public final class Slotomatic {
   ...
   public void runSlotomatic() {
      ...
      runSlotomatic(this.slotomaticOptions, this.slotomaticOptions.getModuleRoot(), this.slotomaticOptions.getPaths());
   }
   ...
   private static void printResults(Compiler compiler) {
      if (logger.isLoggable(Level.FINE)) {
         logger.fine(
            String.format("Compiled %d files, skipped %d files, failed %d files", compiler.getCompileCount(), compiler.getSkipCount(), compiler.getFailCount())
         );
      }
   ...
   static {
      ...
      handler.setFormatter(new Slotomatic.SlotomaticLogFormatter());
      setLogHandler(handler);
   }

   public static class SlotomaticLogFormatter extends Formatter {
      @Override
