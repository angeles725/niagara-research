...
- **Covered blocks (Spyder ecosystem)**: B77 (Spyder BACnet+LON drivers), B96 (Venom TAB),
  ...
  **B121 (Kingfisher / TR wall-module tool-side — closes G7: `BSylkTIDeviceModelEnum` 21-model registry [TR23/40/42/70/71/75 +SIO +actuator];
  ...
  `BKFStateMachine` 3066-line LCD/HMI simulation; CRC-16-only/no-signature confirmed at Kingfisher+Sylk path; escalates multiple B106 `[CERT-a]`→`[CERT]` — NEW this iteration)**.
- **Coverage metric (this focus)**: **7 / 8** Spyder-ecosystem gaps closed this loop session (G1, G2, G3, G4, G5, G6, G7).
- **Last iteration (it.7)**: 2026-06-28 — closed **G7** (Kingfisher / TR wall-module tool-side deep-dive, code-side).
...
- **Open gaps — read-only investigable**: **0 main** + 2 micro-gaps (G4b, G6b — both XML-extract, low pri)  ← STATIC loop main-gap exhaustion reached; only the 2 XML-extract micro-gaps remain
...
- **Total Spyder-ecosystem gaps remaining**: **2** main (0 read-only investigable + 1 requires-execution [G8] + 1 blocked-on-artifact [G5b]) + 2 micro-gaps (G4b, G6b)
- **Next recommended**: with **G7 now closed (→ B121)**, every MAIN read-only-investigable Spyder gap is done. The ONLY remaining read-only-investigable work is the two **XML-extract micro-gaps**: **G4b** (bundled IO XML: `IO_Common.xml`, per-model `*IO.xml`, `lonXL10NextGenFixedIo` → upgrade per-model fixed-point values `[CERT-doc]`→`[CERT]`) and **G6b** (`BacnetSpyder.xml`/`LonSpyder.xml` store offsets/capacities → upgrade Model-8/Micro capacity `[INFER]`→`[CERT]`). Both are low-priority resource-XML extractions, NOT new code distillation. The orchestrator may either (a) run one more thin iteration to absorb G4b+G6b (an existing-block escalation, not a new block) and then STOP, or (b) STOP now and treat G4b/G6b as deferred micro-polish. Blocked **G5b** (needs `tasowizSupport` JAR) and requires-execution **G8** (live IRM/BEATS hardware) remain non-read-only.
  **Read-only investigable Spyder MAIN gaps remaining after this iteration: 0. Only G4b + G6b micro-gaps left.**
