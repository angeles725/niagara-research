...
| S22 | lint-ext-writable-shape per-slot writing-action rule — exempt a complex OPERATOR slot only when an @NiagaraAction BODY writes THAT slot (setX / .set(x,…)), reusing S18's slot→writer follow | KIT | precision: closes the class-level false negative | Med (awk slot→writer follow, bats) | C10 | Evidence: BCompressorControl.faultReset (BStatusBoolean OPERATOR, no writer) exempted by the unrelated hidden powerOnExpired at a109249 — a false negative under the C9 class-level rule; pairs with [[S21]] as a C10 lint-precision cluster (probe 2026-09-06-c9-pr10-green-read.md) |
| S23 | lint-silent-protection surface-follow must recognise a `BIAlarmSource`/`AlarmSupport.newOffnormalAlarm` adapter (and a child `BAlarmSourceExt` on a point the field drives) as a SURFACE | KIT | precision: removes a Pattern-B false positive after alarms ship | Med (awk surface-follow ext) | C10 | Evidence: post-PR9 CP-1 shed on `CompressorControl.java` STILL flagged (lint run @ 4f77dd1 = 1 WARN) although alarmed via `BIAlarmSource` in `BCompressorControl`; CR-3 (child ext) IS recognised. Clusters with [[S21]]/[[S22]] under 'C10 lint precision' |
