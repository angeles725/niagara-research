...
| high | T4 condiciones + neqlize (identificacion inversa) | decompiled-java | closed (B263) |
| medium | T5 haystack la implementacion completa | decompiled-java | closed (B264) |
| medium | T6 exportTags runtime | decompiled-java | closed (B266) |
| medium | T7 exportTags UI + integracion px | decompiled-java | pending (NEXT) |
...
- **read-only-investigable**: **3** (T7, T8, T9) → focus ACTIVO.
- **requires-execution**: 0. **blocked**: 0.
- **Coverage metric**: **7 / 10** (B260-B266). El backlog creció en it.3: +T10 (Brick), cerrado en it.6.
...
| it.6 | 2026-07-24 | **T10** — Brick | **B265** | **Dos clases que arreglan dos límites del framework.** La doc oficial `[CERT-doc]` confirma que el diccionario Brick *"is an instance of the standard Niagara Smart Tag Dictionary and does not contain any custom properties or actions"* y se genera desde un JSON de reglas con override custom del usuario. Costo comparado de las TRES ontologías de N4.14: Niagara ~24 clases · Haystack **37** (35 son tags computados) · **Brick 2**. (1) **`BBrickIdTag`** computa un **UUID v3 name-based** (`UUID.nameUUIDFromBytes` sobre `"<station>:<handleOrd>"`), determinístico y no almacenado, con `validity` fijada a `BControlPoint` — **contraste directo con B264 §264.5-c**: `h4:id` es un handle ORD disfrazado que ningún cliente externo parsea, mientras Brick emite un UUID estándar con una línea. Precio: deriva del handle, así que mover el punto cambia su identidad. (2) **`BBrickInverseRelation`** aporta **el inverso explícito que B262 §262.4 estableció que el framework NO tiene**: un slot `inverseName`, y `addRelations()` busca las relaciones DIRECTAS con el id inverso y emite `BasicRelation` implícitas con la dirección NEGADA (previo chequeo de no-duplicado) ⇒ una relación unidireccional almacenada se vuelve un par bidireccional consultable **sin almacenar nada**. Y es serializable (`encodeToJson`/`decodeFromJson` incluyen `inverseName`), que es lo que permite declarar pares inversos desde el JSON del generador sin escribir Java. **Lectura transversal**: el punto de extensión que se usa en la práctica es la SUBCLASE de `BTagInfo`/`BRelationInfo` montada en un diccionario, no el registro de agentes — coincide con lo que B254 §254.3 observó en el charting. Back-fill de `SOURCES.md`: `docTagging-N4.14-guide.md` → B265 | **no · inline** (2 clases, 145 líneas) |
| it.5 | 2026-07-24 | **T5** — haystack | **B264** | **El módulo `haystack` NO es un servidor ni un cliente: son DOS DICCIONARIOS** — `BHsTagDictionary` (namespace `hs`, Haystack 3) y `BHaystack4TagDictionary` (`h4`, Haystack 4), ambos `BSmartTagDictionary`, o sea instancias del motor genérico de B260/B261. **Ausencia probada con un matiz fino**: el barrido de `servlet|hisRead|watchSub|pointWrite|zinc` sobre las 37 clases da **una** coincidencia y NO es código — es `defs.json`, el recurso de la ontología, que define esas ops como CONCEPTOS del estándar. El módulo conoce los nombres de las ops porque están en la ontología, pero **no implementa ninguna**: instalar `haystack` NO expone la station a clientes Haystack (da vocabulario, no interoperabilidad). Ontología EMBEBIDA como datos: `tags.csv` 233 líneas + `equip.csv` 229 (hs) y `defs.json` **497 KB** / 658 defs + `protos.json` (h4), con merge files del usuario que pueden sobreescribir el mapeo. **35 de las 37 clases son tags COMPUTADOS** (curVal/curStatus/kind/unit/writeLevel 1-17/his*/equipRef...) — mismo patrón que el namespace `n`. Único chequeo de permisos del módulo: `checkIsSuperUser()` en el job de migración hs→h4. **3 DEFECTOS SEMÁNTICOS**: (a) **`tz` es la zona horaria de la JVM**, igual para toda la station — en Haystack `tz` es propiedad de la ENTIDAD, así que una integración multi-sitio reporta mal; (b) `equipRef` tiene DOS implementaciones con criterios distintos (`BEquipRefTag` busca `BDevice` ancestro; `BEquipRelation` busca el marcador `equip`); (c) `h4:id` es un handle ORD de Niagara con `$`→`~`, NO un `@ref` estándar → un cliente externo no lo parsea. Sin gates de licencia (el licenciamiento está en el servicio, B260 §260.2). 9 tokens re-verificados | sí · **sonnet** (53 tool-calls) + verificación inline |
| it.4 | 2026-07-24 | **T4** — condiciones + neqlize | **B263** | **CORRIGE MI PROPIA PREMISA DEL BACKLOG**: sembré T4 como "neqlize = tag→query" y es FALSO. `neqlize` **no traduce condiciones a NEQL**: es **identificación INVERSA** — dado un componente objetivo, `BNeqlizeRpc.getIdentifyingTagsRelations` (RPC de Fox) busca el conjunto MÍNIMO de tags que lo identifica unívocamente en un espacio de búsqueda (`TagsCandidateIterator` recorre el power-set en orden de tamaño creciente y devuelve el primer subconjunto que ningún par comparte). Es el motor del "armame la query" de la UI: señalás un punto y te devuelve el `hasTags(...)` mínimo. Álgebra: `BAnd`/`BOr`/`BNot` (+`BAlways`/`BNever`) sobre **4 hojas**, y **3 de las 4 terminan en NEQL** (`BBooleanFilter` y sus subclases `BHasAncestor`/`BHasRelation`) — la condición no es un DSL propio, es NEQL embebido en un árbol de componentes. **El EXPORT es más restrictivo que el runtime**: en runtime anidan sin límite, en CSV cualquier `BAnd` descendiente o >1 `BOr` → rechazado ⇒ hay condiciones válidas y funcionales que NO se pueden exportar. `testIdealMatch(Type)` permite pre-filtrar la paleta de tags por TIPO sin instancia, pero **`BBooleanFilter` devuelve `true` siempre** ⇒ el pre-filtro queda desactivado justo para las condiciones más usadas. Exclusiones (`n:name`, `n:displayName`, `n:parent`, `n:child`, `n:tagGroup`...): fuerzan un identificador SEMÁNTICO, no nominal ni estructural. **`BHasAncestor` NO tiene guarda de ciclos** (simple `for` sobre `getParent()`, que cae también en la relación `n:parent` creable a mano) mientras su hermana `BHasRelation` SÍ usa `HashSet visited` ⇒ un ciclo de `n:parent` la haría desbordar la pila. §263.7: **acoté 1 afirmación del barrido** — la RPC es `permissions="unrestricted"` PERO filtra cada objetivo por `hasOperatorRead()` del contexto; reportarla sin eso habría sido falsa alarma. 8 tokens re-verificados | sí · **sonnet** (15 clases) + verificación inline |
