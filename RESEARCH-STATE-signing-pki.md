...
| 4 | **SP-G10a** — License-side mirror runtime confirm: re-run the Java-layer hook (`LicenseUtil.verify`/`Signature.verify` → force true) with a FULL frida agent (Java bridge) on a disposable `nre.exe`. Static Java path is pinned (B524 F1); the runtime forcing is the remaining unproven half. | blocked-on-tool (frida Java bridge cannot auto-attach: `nre.exe` is an embedded-JVM launcher, no standard `jvm.dll`) | open (B524) — §21.4 provisioning ruler (options A/B) in `sources/probes/B524-spg10-frida-2026-08-25/SP-G10a-blocked-on-tool.md` |
