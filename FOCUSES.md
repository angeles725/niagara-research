...
|---|---|---|---|---|
| **jace-history-audit** | **active (0/5, HD1–HD5; bootstrapped 2026-08-30)** | `RESEARCH-STATE-jace-history-audit.md` | El TRACE operativo real de la seed station JACE_UMBRELLA: qué grabaron sus stores de history/audit/alarm, leído del SD (`local-sd-image/`, P2). `live-install` → SECRETS DISCIPLINE (schema/counts/tipos; identidades enmascaradas). **Ángulo:** NO el formato .hdb (ya en focus `database` B402-413) sino el CONTENIDO + un lector .hdb propio (§19). 5 gaps: HD1 formato+lector · HD2 Audit/Security content · HD3 LogHistory (55KB) · HD4 alarm.adb · HD5 provisioning .hdb + síntesis. REMITTANCE: modelo .hdb database B402-413, config desplegada B689. NEXT HD1→B699. | B699– |
