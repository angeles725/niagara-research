...
| **database** | **bootstrapped (0/10)** | `RESEARCH-STATE-database.md` | La capa de PERSISTENCIA de N4 como subsistema — no el formato de cada archivo (ya en B5/B33/B34/B114/B393) sino la mecánica viva que el corpus nunca abrió: ciclo de guardado del BOG (trigger/dirty flag), modelo BComponentSpace/BSpace, ejecución BQL contra el space, migración de BOG entre versiones, y sobre todo el EXPORT a RDBMS externo (rdb-rt, alarmOrion, HSQLDB embebido) — el puente base-interna↔SQL-externa, hoy con CERO bloques. 10 gaps DB1-DB10 (audit-first 2026-08-09). NEXT DB1 | (ninguno aún) |
...
| platform-native | **stopped (13/13, investigable=0)** | `RESEARCH-STATE-platform-native.md` | RE nativo de la plataforma (launchers, JNI, licensing/crypto, driver DLLs, daemon). Base estática B124-B130 (grado strings/RTTI). **Sub-pass Ghidra-grade 2026-08-07 (B379-B383)**: decompilación de CUERPOS de función — nverify (4 flags skip-* + pin TPK RSA-2048 por memcmp), njre launcher (provider FIPS-gated + gate anti-agente), plat.exe (daemon LocalSystem/auto-start + passphrase argv + policy + DPAPI REG_BINARY/HKLM), libciper.so (Sylk masterslave file-transfer + dual CRC, no crypto), síntesis B383. NG1-G1 (B384: gate sites, --skip-signature-check salta todo el verify) + NG2b (B385: 107-native taxonomy, add-user/exec-command son STUBS return-0 en Windows) CERRADOS. **STOPPED investigable=0** | B124–B130, **B379–B385** |
| **license-diff** | **stopped (6/6, investigable=0)** | `RESEARCH-STATE-license-diff.md` | Diff LICENCIADA vs SIN-LICENCIA en disco con diffoscope+SHA-256, para documentar qué cambia un license de Niagara en el filesystem. Par: A=Honeywell OptimizerSupervisor **4.14.0.162 licenciada** (3 .license, ya analizada B124-B385) vs B=Tridium EMEA Supervisor **4.15.3.28.2 sin-licencia** (0 .license). CAVEAT 3 EJES: licencia + versión(4.14→4.15) + vendor(Honeywell OEM→Tridium base) — cada hallazgo se atribuye por eje. 6 gaps L1-L6 CERRADOS (B386-B389). RESPUESTA: un license cambia SOLO security/ en disco (B386) + gates de feature en runtime (B387); módulos/bin/config difieren por vendor/versión/usuario (B388/B389). Hallazgos: sin-licencia corre UNCAPPED (límites→MAX_VALUE) no deshabilitado, heap-limit→exit(-3); firma verificada por 2 master keys DSA+ECDSA hardcodeadas (confirma B126 §126.6); launcher nativo nre/njre/station/plat byte-idéntico 4.13↔4.14. diffoscope 327 en venv del kit | B386–B389 |
