...
|---|---|---|---|---|
| **jace-station-config** | **active (0/8, SC1–SC8; bootstrapped 2026-08-30)** | `RESEARCH-STATE-jace-station-config.md` | La CONFIGURACIÓN REAL DESPLEGADA del controlador de campo JACE-8000 (station `JACE_UMBRELLA`), leída del `config.bog` del microSD READ-ONLY (`local-sd-image/`, P2 QNX6 via `tools/qnx6read.py`). Data-at-rest OFFLINE, no hardware vivo. `live-install` → SECRETS DISCIPLINE (estructura, no valores). **Ángulo:** NO los internals del framework (ya cubiertos) sino QUÉ corre esta unidad y en qué difiere del SUPERVISOR. `config.bog` = BOG XML EN CLARO (ZIP+file.xml, 51 KB); solo campos `BPassword` cifrados GCM (B393). Skeleton medido en bootstrap: station FIPS=off/keyring, **23 Services**, **2 redes** (NiagaraNetwork `nd` upstream + NrioNetwork `nrio` field IO), tagging pesado (`td:` ×144). 8 gaps: SC1 skeleton+Services · SC2 NiagaraNetwork (¿subordinada del supervisor?) · SC3 NrioNetwork (IO físico) · SC4 RBAC desplegado · SC5 alarmas/historias/audit · SC6 tags/hierarchy · SC7 servicios plataforma/orquestación+Program · SC8 stores+síntesis vs supervisor. REMITTANCE: BOG format B15/B33, este .bog vivo B473 / SD B674, cifrado reposo B393/B466, NRIO driver B680, NiagaraNetwork B414-420, RBAC B558-566, tags B260-270, hierarchy B584-590. NEXT SC1→B685. | B685– |
