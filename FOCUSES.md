...
| **electronicSignature** | **active (1/7)** | `RESEARCH-STATE-electronicSignature.md` | El add-on **TridiumPS** `electronicSignature` (+ `electronicSignatureRemote`), namespaces `com.tridiumx.ps.*` + `com.secured.*`, como la capa de firma electrónica **21 CFR Part 11**: un punto se asegura por SUSTITUCIÓN DE TIPO (`BSecured*Writable`) y cada verbo de escritura exige su gemelo firmado `*WithAuthentication` con re-autenticación (LDAP/local), razón-de-cambio obligatoria (`BReasons`/`BReasonSet`), segundo firmante opcional (`BSecondaryRemoteAuthentication` + `BSecureUserMixIn` Level-2), y registro de auditoría (`BSecuredTrendRecord` en history). Gate `tridium:eSignature` + `point.limit`. Declaración §11.100(c) legalmente vinculante en lexicon. **Refuta** la creencia de que `signingService` (PKI) cumplía Part 11 (B350 §350.6). **Ofuscación**: decompilado string-scrubbed, bytecode/resources intactos. Bootstrap audit-first 2026-08-04. 39 tipos registrados | B350– |
...
**electronicSignature** (ACTIVE 1/7) — bootstrapeado 2026-08-04. B350 (ES1) es el bloque fundacional: identidad
(vendor **TridiumPS**, v4.14.1.30.11, símbolo `esign`), 39 tipos registrados, gate `tridium:eSignature` + `point.limit`,
y el mapeo 21 CFR Part 11 (certificación §11.100(c) + set `*WithAuthentication` + reasons + segundo firmante + audit
`BSecuredTrendRecord`). 6 gaps abiertos investigables: ES2 flujo de firma · ES3 dual/remote transport · ES4 protección
del audit trail + purga sin firma (riesgo §11.10(e)) · ES5 credenciales (Base64/sleep, confirmar en bytecode) · ES6
capas ux/wb · ES7 ESignAcknowledgement mutable. Próximo bloque: **B351**. Ver `RESEARCH-STATE-electronicSignature.md`.
