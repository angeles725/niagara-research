...
| **api-access** | **document 1/1** | `RESEARCH-STATE-api-access.md` | DOCUMENT-mode (§20): método reproducible de login programático a una estación N4 viva (estación propia del operador, cuenta `API2`, acceso legítimo). GET /prelogin → SCRAM-SHA-256 → `acceptEula` → oBIX/BQL. `[CERT]` código + `[CERT-live]` cross-session (3 vías). Tools de referencia en `sources/probes/B457-n4-login/`. Acción: rotar credenciales expuestas | B457 |
