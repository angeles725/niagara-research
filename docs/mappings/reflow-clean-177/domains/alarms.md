...
- **[FIXED-69] AlarmSubscriber pattern (corrección audit empírico)**: el frontend **NO usa push WebSocket**. Es **polling con `setInterval` en componentes**: `AlarmDisplay.vue:168` (`setInterval(getAlarmCount, rate*1000)` con `rate = consoleRefreshRate || 20`), `AlarmsHome.vue:359`, `AlarmDetails.vue:451`. `$niagara.alarmSubscribe` **NO existe** — el namespace alarm de `niagara.js` (mock) tiene 20 métodos sin `subscribe`. `BReflowChannelService` es pub-sub **genérico** (channels `'reflow'` route-sync entre clientes + `'reflow-config'` config-control lock), **NO emite alarmas**. Cross-ref bloque #69 §69.2 + §69.4 + §69.8. La síntesis previa del mapping (push reactivo vía ChannelService) era inferencia incorrecta del GAP-ANALYSIS — el audit empírico directo sobre `Reflow-Clean-177/` (READ-ONLY) cerró flag #241.
- **[OPEN] WebSocket commented stub (Phase 5+)**: el cliente WebSocket (`api/websocket.js`) está comentado en `main.js:107`; en producción real, si esa capa se reactivase, la arquitectura podría evolucionar — pero hoy `Reflow-Clean-177/` no está conectado a backend (mock). MX60 debe decidir activación WS sprint 1 (decisión #244..#248).
