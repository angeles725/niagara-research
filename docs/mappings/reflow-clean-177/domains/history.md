...
- **[FIXED-69] HistoryGhostSubscriber pattern (corrección audit empírico)**: **NO es keepalive**. El código real (26 LOC, lectura directa) muestra **subscriber de detección one-shot con auto-unsuscribe al primer evento** (líneas 19-21: `event() → unsubscribe(this, this.subscriberCtx)`). Su rol es disparar una vez ante el primer evento de history y desuscribirse. El **verdadero keepalive de subscriptions** es `lib/bajaHeartbeat.js` (`LEASE_MS=30s` + `RENEWAL_MS=20s`), pero está **DORMANT en clean-room** — `start(baja)` nunca es llamado (`main.js` no lo invoca). MX60 debe activar `bajaHeartbeat.start(baja)` desde sprint 1 (sino tabs en background pierden subscriptions silenciosamente). Cross-ref bloque #69 §69.5 + §69.6. La síntesis previa del mapping (mantener viva la sesión durante queries pesados) era inferencia incorrecta del nombre — el audit empírico cerró el gap.
