...
| greenmax-tablero-gen.py | tools/greenmax-tablero-gen.py | created (2026-09-11) — HARBOR GreenMAX tablero Px transform. Takes an ORIGINAL tablero `.px` and emits the REDESIGNED one (name-selector) as fenced-XML Markdown, ready to paste into the PxView inside `GM<X>ilum`. AUTO-DETECTS circuit count + pill positions from the Px (don't trust file names: `GreenMAX07 TabAL8.px` is GM08's 32-relay view). Removes HOA buttons + calendar popups, KEEPS pill (Relay[k].BO value) + LED (Relay[k].BO status), APPENDS a `NameMux{k}`+`SelR{k}/set` selector at each pill's (x, y+30) with ABSOLUTE `station:\|slot:/Iluminacion/<gm>/…` ords (control folder = plain number, no leading zero even when the device is `GM0Xilum`). `--legend` adds the read-only "Horarios" (1..5 + live shared name) legend; validates XML; bumps `viewSize` if needed. Used to replicate the redesign to all 13 operative tableros (GM02,04-08,10-16). `--help` for usage. |
...
| bog-nav.py | tools/bog-nav.py | created (2026-09-05) — READ-ONLY navigator for a station `config.bog` (ZIP+file.xml BOG-XML). Walks the component tree, resolves `sourceOrd='h:xxxx'` link handles to component PATHS (answers "which link feeds Cuarto1.setpoint?" → `Cuarto1.setpoint --> Programacion/ColdRoom_1.setpoint`), classifies each value slot by external-write shape (StatusNumeric=complex, child-leaf bare `<real>` preferred per B826/B825; plain=simple; untyped=bare), and diffs two bogs. Grammar + handle-graph parser REUSED from `build-n4-module-kit/toolbelt/bog-audit.sh` (kit main 3f666a0). Subcommands: tree/slot/links/handle/path/find/writable/relays/hoa/tiles/grep/diff/selftest. `relays` = CHECK11 (own-module output→writable proxy, fallback+writeOnUp; PANCCADIA 22 targets, 17 no-fallback); `hoa` = CHECK8 (mode/HOA slots, auto vs override; 19 auto); `tiles` = CHECK18 (per-RoomPanel evapN tile→unit, flags Cuarto1 units 1/3 crossed); `links --dangling --src` = CHECK7 (tgt slot absent from source). `--slot` is exact and ENDPOINT-AWARE (2026-09-06): with `--from` it names the source slot, with `--to` the target slot, alone either end; `--slot-any` restores either-end matching (the pre-fix behaviour, which made `--from Cuarto1 --slot fanMode` return the `evapNFanMode --> …fanMode` links). Reads `.bog` and a `.dist` station backup (nested config.bog). stdlib, `--json`/`--csv` on every command, `selftest` (24). |
