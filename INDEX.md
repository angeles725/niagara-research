...
Este índice te guía entre los **51 bloques** de investigación (41 del framework + 7 del stack frontend SPA externa + 2 audit Reflow + 1 nuevo Bloque 49 Facets+i18n cliente que CIERRA Capa 16 completa). Cada bloque es un archivo `.md` independiente que puede leerse aislado, pero las conexiones están explícitamente marcadas entre sí.
...
### Capa 17 — Reflow audit cross-stack (Bloques 50, 51, 53-60) — código real producción + dominios cliente↔server + HTTP infrastructure + WebSocket layer + sync engine + responses outliers + backups
...
├── niagara-mental-model-bloque42.md  (bajaScript Subscriber lifecycle end-to-end + lease 10s + reconnect manual + leaks)
├── niagara-mental-model-bloque44.md  (Alarm Console pipeline frontend: BAlarmSpaceConnection + canal BOX alarm + ack/clear)
├── niagara-mental-model-bloque45.md  (History/Trend chart consumption: WebChart 3 GET + NDJSON + boxcs real-time tail)
├── niagara-mental-model-bloque46.md  (Writes priority array desde SPA externa: oBIX REST + 3 niveles + Transaction NO ACID)
├── niagara-mental-model-bloque47.md  (Bootstrap headless SPA externa: location.host hardcoded + CORS ausente + SCRAM)
├── niagara-mental-model-bloque50.md  (Reflow-Clean-177 audit Par A: frontend ↔ -ux — -ux solo iframe loader)
├── niagara-mental-model-bloque51.md  (Reflow-Clean-177 audit Par A real: frontend ↔ -rt + app-readable.js deobfuscado)
├── niagara-mental-model-bloque48.md  (RBAC visibility frontend: bajaScript NO expone canRead/canWrite/canInvoke + slot pruning automático + canX() server calls + Reflow profiles role-name + LEVEL_1 sin enforcement per-priority)
├── niagara-mental-model-bloque49.md  (Facets + i18n + formatting cliente: BFacets browser API + BEnumRange + getDisplay vs cliente formatting + BAbsTime+BTimeZone + lexicons fallback chain + Reflow ignora BTimeZone single-locale)
├── niagara-mental-model-bloque52.md  (CSRF cross-frame token injection SPA-in-iframe: Plan E `window.top.document` primario + Plan A `/login` fallback REFUTADO post-auth)
