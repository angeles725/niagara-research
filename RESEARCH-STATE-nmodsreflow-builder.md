...
- **Métrica**: base **12 / 12 CERRADO** (BG1-BG11 + BG13). **REABIERTO 2026-07-12** (autorizado por usuario) con
  ...
  **D** (pedido usuario 2026-07-12) módulos que usa + sistema de vistas Workbench/navegador. Métrica reapertura: **14 / 16 — grupos A+B+D+E COMPLETOS (B228-B241). SOLO falta grupo C (dinámico BG20/BG21), PENDIENTE de OK del usuario. Grupo C PENDIENTE de OK del usuario.**
...
| — | BG24 · **UI de alarmas cliente**: AlarmDisplay widget (count poll consoleRefreshRate); página consola (getAlarmList filtros/columnas per-console); **ack/ackAll vía Baja NATIVO** ($baja Ord.make alarm: ackAlarms, bypasa BReflowAlarmCommands); alarmas audibles (checkAlarmSounds→new Audio, sound-library por prioridad); store alarms(consoles)+alarmData | SPA (beautify) | **cerrado B240** |
| — | BG25 · **Schedules (frontend)**: widget `schedule-list` suscribe ORDs pre-elegidos (no BQL); edición=**WebScheduler NATIVO** ($niagara.browser.open view:schedule:WebScheduler/WebCalendarScheduler, no editor propio); página global BQL discovery + hx embed; store schedules/scheduleData | SPA | **cerrado B241** |
