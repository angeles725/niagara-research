...
|---|---|---|---|---|
| 1 | MED | **`chihuahua-rt` AND `chihuahua-ux` declare `<niagara-permission-groups type="all"/>+workbench+station`** | Tridium `control-rt` declares NO `<permissions>` → minimal base grant ([B635]) | Remove `<permissions>` (fall to base grant) or scope to the one group actually needed. Under the default `GrantAllPermissionGroupStore` this is granted anyway ([B635]) so it is not an active escalation — but it is maximal-intent where minimal was correct, and becomes real over-privilege the day a restrictive store is deployed. It requests NO `<java-permissions>` (good: `checkTpk` stays false). **§14 REFINED by [B649]**: at SOURCE these `type="all"` groups are the untouched Tridium scaffold — EMPTY (placeholder comments only, `<req-permission>` commented out) — so they request nothing beyond the base grant; effective posture is already minimal. Downgrade to cosmetic cleanup (delete the empty scaffold), not a privilege fix. |
