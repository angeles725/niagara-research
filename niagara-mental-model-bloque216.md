...
| **OpenCSV** (`com.opencsv`) | `commands/BReflowCSVCommands.java` + `AlarmCSVResponse.java` | 2 | ⚠ **CORREGIDO en B237 (§14)**: NO exporta history. Se usa para (a) IMPORTAR el point-map CSV (`loadPointMap`, wizard de auto-binding B228) y (b) generar el CSV de ALARMAS server-side. El export de history a CSV es 100% client-side (`data:text/csv`, sin opencsv). |
