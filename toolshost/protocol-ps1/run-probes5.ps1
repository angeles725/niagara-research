...
foreach ($n in @('probe5-gen-plus1d','probe5-gen-plus2d')) {
  Copy-Item "$staging\$n.license" "$licDir\" -Force
}
$out = & 'C:\Niagara\iC-Niagara-4.10.9.14\bin\nre.exe' -licenses 2>&1 | Out-String
$out -split "`n" | Where-Object { $_ -match 'probe5|invalid|valid|moved|Feature|generated|signature|Signature' } | ForEach-Object { Write-Output ('RES ' + $_) }
foreach ($n in @('probe5-gen-plus1d','probe5-gen-plus2d')) { Remove-Item "$licDir\$n.license" -Force -ErrorAction SilentlyContinue }
