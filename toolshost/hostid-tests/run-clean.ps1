...
Copy-Item "$staging\HONEYWELL-REHOSTED-CLEAN.license" "$licDir\" -Force
$out = & 'C:\Niagara\iC-Niagara-4.10.9.14\bin\nre.exe' -licenses 2>&1 | Out-String
$out -split "`n" | Where-Object { $_ -match 'CLEAN|invalid|valid|Features|moved|ADVERT|Signature|decoding|Honeywell' } | ForEach-Object { Write-Output ('RES ' + $_) }
Remove-Item "$licDir\HONEYWELL-REHOSTED-CLEAN.license" -Force -ErrorAction SilentlyContinue
